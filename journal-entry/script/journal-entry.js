...
//Double Clicks
	...
	$(document.body).on('dblclick', '#tblGL tbody > tr', function () 
	{
		...
		if(type == 'ACCT'){
			...
			$('.selected-det').find('input.shortname').val(code);
			$('.selected-det').find('input.type').val(type);
		}
		else{
			$('.selected-det').find('input.glaccount').val(code);
			$('.selected-det').find('input.glname').val(name);
    ...
    $(document.body).on('dblclick', '#tblGLControlAccount tbody > tr', function () 
	{
		...
		$('.btnrowfunctions').removeClass('d-none');
        $('#glControlAccountModal').modal('hide');
...
//Submit
	...
	$(document.body).on('click', '#btnAdd', function () 
	{
        ...
        if($('#tblDetails tbody tr').find('input.glaccount').val() == '' ){
			...
			$('#messageBar2').addClass('d-none');
						$('#messageBar3').removeClass('d-none');
						$('#messageBar').text('Select Branch first!').css({'background-color': 'red', 'color': 'white'});
        ...
        var tbl = $('#tblDetails tbody tr').each(function (i) 
		{
            ...
            var itArr = [];
				...
				if ($(this).find('input.glaccount').val() != ''){
					...
					itArr.push('"' + $(this).find('input.branchname').val().replace(/,/g, '') + '"');
					itArr.push('"' + $(this).find('input.department').val().replace(/,/g, '') + '"');
					itArr.push('"' + $(this).find('input.branchocrcode2').val().replace(/,/g, '') + '"');
					itArr.push('"' + $(this).find('input.employeeorccode3').val().replace(/,/g, '') + '"');
					itArr.push('"' + $(this).find('input.shortname').val().replace(/,/g, '') + '"');
					itArr.push('"' + $(this).find('input.type').val().replace(/,/g, '') + '"');
...
//Rows
	...
	//Branch
	$(document.body).on('change','.bplid',function()
	{
		
		let branchname = $(this).find('option:selected').attr('val-rate');
	
		$('.selected-det').find('input.branchname').val(branchname);
